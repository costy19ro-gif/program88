...
def verifica_ligi_active(league_ids: list[int], zile_inainte: int = 3) -> list[dict]:
    """
    Verificare rapida: pentru fiecare liga, cate meciuri viitoare (neterminate)
    are in urmatoarele `zile_inainte` zile — FARA sa ruleze motorul Poisson
    (mult mai rapid decat scaneaza(), util ca sa vezi ce liga sa scanezi
    efectiv). Foloseste acelasi cache 6h ca restul aplicatiei, deci verificarea
    nu costa cota suplimentara daca urmeaza sa scanezi aceleasi ligi dupa.
    """
    azi = date.today()
    prag = azi + timedelta(days=zile_inainte)
    ...
    for lid in league_ids:
        try:
            meciuri = data_source.meciuri_liga(lid)
        except RuntimeError as e:
            rezultate.append({"league_id": lid, "meciuri_viitoare": None, "eroare": str(e)})
            continue
        ...
        rezultate.append({"league_id": lid, "meciuri_viitoare": nr, "eroare": None})
    return rezultate
