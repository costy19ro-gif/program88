...
with tab_poisson:
    ...
    if meciuri:
        ...
        st.dataframe(
            df_meciuri[["fixture_id", "liga", "tara", "echipa_gazda", "echipa_oaspete", "status"]],
            use_container_width=True,
        )

        optiuni = {
            f"{m['echipa_gazda']} — {m['echipa_oaspete']} ({m['liga']})": m
            for m in meciuri
        }
    ...
    if "ultima_analiza" in st.session_state:
        ...
        # Bonus: predictia proprie API-Football, doar pentru comparatie
        with st.expander("🔍 Comparatie cu predictia API-Football (bonus)"):
            try:
                pred = data_source.predictie_oficiala(meci["fixture_id"])
                if pred:
                    st.json(pred)
                else:
                    st.info("Nu exista predictie oficiala disponibila pentru acest meci.")
            except RuntimeError as e:
                st.warning(f"Nu am putut lua predictia bonus: {e}")
