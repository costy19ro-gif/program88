...
with tab_scanner:
    ...
    st.caption(
        "⚠️ In perioada de vara (iulie-inceput august), ligile mari din Europa "
        "de obicei nu au meciuri programate (pauza intre sezoane) — '0 meciuri "
        "gasite' poate fi normal, nu neaparat o eroare. Incearca o liga aflata "
        "in sezon activ (ex. Liga I Romania incepe de obicei in iulie)."
    )
    ...
    if not ligi:
            ...
            for liga in tara.get("leagues", []):
                for tara_tinta, liga_tinta in LIGI_IMPLICITE:
                    if tara.get("name") == tara_tinta and liga.get("name") == liga_tinta:
                        implicite.append(f"{tara.get('name')} — {liga.get('name')}")
        ...
        if st.button("🔍 Verifica ce ligi (din cele alese) au meciuri active", key="btn_verifica_ligi"):
            if not ligi_alese:
                st.error("Alege cel putin o liga mai intai.")
            else:
                league_ids = [optiuni_liga[k] for k in ligi_alese]
                with st.spinner("Verific fiecare liga..."):
                    verificare = scanner.verifica_ligi_active(league_ids, zile_inainte=zile)
                id_catre_eticheta = {v: k for k, v in optiuni_liga.items()}
                randuri = []
                for r in verificare:
                    randuri.append({
                        "Liga": id_catre_eticheta.get(r["league_id"], str(r["league_id"])),
                        "Meciuri viitoare": r["meciuri_viitoare"] if r["eroare"] is None else "eroare",
                    })
                df_verificare = pd.DataFrame(randuri).sort_values("Meciuri viitoare", ascending=False, key=lambda s: s.map(lambda v: v if isinstance(v, int) else -1))
                st.dataframe(df_verificare, use_container_width=True, hide_index=True)
                st.caption("Alege pentru scanarea completa doar ligile cu meciuri viitoare > 0.")
