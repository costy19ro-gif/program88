...
with tab_scanner:
    ...
    st.caption(
        "Cauta meciuri viitoare din doua surse, ruleaza motorul Poisson pe "
        "fiecare, si construieste automat categoriile de bilet. Prima "
        "repriza nu e folosita inca — categoria a fost eliminata din bilet."
    )

    st.markdown("#### Sursa 1 — RapidAPI (toate ligile, scanate pe interval de date)")
    st.caption(
        "Nu mai trebuie sa alegi ligi — scanam automat toate ligile active "
        "in intervalul de date de mai jos, dintr-un singur set de apeluri "
        "(un apel per zi, cache 6h)."
    )
    scaneaza_rapidapi = st.checkbox("Include RapidAPI in scanare", value=True, key="scaneaza_rapidapi")
    zile_istoric = st.slider(
        "Cate zile in urma sa caut istoric per echipa (RapidAPI)?", 14, 90, 45,
        key="zile_istoric_rapidapi",
        help="Mai multe zile = istoric mai bun, dar mai multe apeluri API.",
    )
    ...
    if st.button("🔍 Verifica sursele alese (meciuri viitoare)", key="btn_verifica_surse"):
        randuri = []
        if scaneaza_rapidapi:
            with st.spinner("Verific RapidAPI (toate ligile)..."):
                v = scanner.verifica_rapidapi(zile_inainte=zile)
            randuri.append({
                "Sursa": "RapidAPI", "Liga/Competiție": "toate ligile",
                "Meciuri viitoare": v["meciuri_viitoare"] if v["eroare"] is None else "eroare",
                "Detaliu eroare": v["eroare"] or "",
            })
        if competitii_alese:
            with st.spinner("Verific football-data.org..."):
                verificare_fd = scanner.verifica_surse_fd(competitii_alese, zile_inainte=zile)
            for r in verificare_fd:
                randuri.append({
                    "Sursa": "football-data.org",
                    "Liga/Competiție": fdo.COMPETITII_GRATUITE.get(r["cod"], r["cod"]),
        ...
        if randuri:
            ...
            st.error("Alege cel putin o sursa (RapidAPI sau o competiție).")
    ...
    if st.button("🎰 Scaneaza și construiește biletele", type="primary"):
        if not scaneaza_rapidapi and not competitii_alese:
            st.error("Alege cel putin o sursa (RapidAPI sau o competiție).")
        else:
            nr_pasi = (1 if scaneaza_rapidapi else 0) + len(competitii_alese)
            ...
            candidati = scanner.scaneaza(
                surse_fd=competitii_alese, scaneaza_rapidapi=scaneaza_rapidapi,
                zile_inainte=zile, zile_istoric_rapidapi=zile_istoric,
            ...
            st.success(f"Am gasit {len(candidati)} meciuri viitoare cu istoric suficient, din {nr_pasi} surse scanate.")
