...
def meciuri_pe_data_toate_ligile(zi: date) -> list[dict]:
    """
    Toate meciurile dintr-o zi, din TOATE ligile active, dintr-un singur apel.
    Foloseste endpoint-ul football-get-matches-by-date-and-league (in ciuda
    numelui, ia doar parametrul `date` — nu are nevoie de leagueid).

    De ce asta si nu meciuri_liga(): football-get-all-matches-by-league are
    gauri reale de acoperire pe multe ligi (verificat — Norvegia, Suedia
    intorc liste goale acolo, desi au meciuri reale azi). Endpoint-ul pe
    data e mult mai complet — grupeaza pe liga, cu ID-uri mici (acelasi
    sistem ca meciuri_liga), deci compatibil cu istoric_echipa_din_liga().
    """
    raw = _get_client().get(
        "football-get-matches-by-date-and-league", {"date": zi.strftime("%Y%m%d")}
    )
    if raw.get("status") != "success":
        raise RuntimeError(f"API-ul a raspuns neasteptat pentru data {zi}: {raw}")

    meciuri = []
    for liga in raw.get("response", []):
...
def meciuri_interval(zi_start: date, zi_end: date) -> list[dict]:
    """
    Toate meciurile intr-un interval de zile (inclusiv), din toate ligile —
    un apel per zi (cache 6h fiecare). O zi care esueaza nu opreste restul.
    """
    toate = []
    zi = zi_start
    while zi <= zi_end:
        try:
            toate.extend(meciuri_pe_data_toate_ligile(zi))
        except RuntimeError:
            pass
        zi += timedelta(days=1)
    return toate
